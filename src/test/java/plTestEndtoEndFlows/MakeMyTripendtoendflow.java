...
public class MakeMyTripendtoendflow extends BaseClass {
	...
	public Map<Integer, ArrayList<String>> MakemytripSuccessfulScenarioCode() throws Exception {
		...
		((JavascriptExecutor) driver).executeScript("arguments[0].click();",
				driver.findElement(By.xpath("//a[contains(@href,'bus-tickets')]")));

		wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//p[@data-cy=\"departureDate\"]")));

		((JavascriptExecutor) driver).executeScript("arguments[0].click();",
				driver.findElement(By.xpath("//p[@data-cy=\"departureDate\"]")));

		wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("div[aria-label='Tue Jan 10 2023']")));

		((JavascriptExecutor) driver).executeScript("arguments[0].click();",
				driver.findElement(By.cssSelector("div[aria-label='Tue Jan 10 2023']")));

		wait.until(ExpectedConditions.elementToBeClickable(By.cssSelector("button[id='search_button']")));

		((JavascriptExecutor) driver).executeScript("arguments[0].click();",
				driver.findElement(By.cssSelector("button[id='search_button']")));
		...
		for (int i = 0; i < payMethodList.size(); i++) {
			...
			if (payMethodName.contains("Pay Later")) {
				...
				if (driver.findElement(By.xpath("//button[contains(@class,'prime__btn')]")).isEnabled()) {
				...
				wait.until(ExpectedConditions
						.elementToBeClickable(By.xpath("//span[@class='lato-bold font16' and text()='SIMPL']")));

				String actualtext = driver.findElement(By.xpath("//span[@class='lato-bold font16' and text()='SIMPL']"))
						.getText();
